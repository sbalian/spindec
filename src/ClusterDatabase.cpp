...
ClusterDatabase::ClusterDatabase() : max_order_(0), cluster_cutoff_(0.0)
{
}

ClusterDatabase::ClusterDatabase(const SpinBath& spin_bath, const UInt max_order,
    const double cluster_cutoff,const string& build_method) :
        max_order_(max_order),spin_bath_(spin_bath),
        cluster_cutoff_(cluster_cutoff)
{
  
  if (build_method == "global") {
    build_with_global_cutoff();
  } else {
    build_with_local_cutoff();
    if (build_method != "local") {
      Errors::warning("Build method not recognized. Using \"local\".");
    }
  }
  
}

void ClusterDatabase::build_ones()
{
...
void ClusterDatabase::build_pairs()
{
...
void ClusterDatabase::build_with_global_cutoff()
{
    ...
    for (UInt j=0;j<num_prev_clusters;j++) {
      ...
      for (UInt k=0;k<num_sites;k++) {
        ...
        if (found_duplicate == true) {
          ...
          // if at least one separation below threshold,
...
void ClusterDatabase::build_with_local_cutoff()
{
